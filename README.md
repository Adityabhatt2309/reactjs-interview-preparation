...
Alright 🔥
Below is the **COMPLETE REACT JS TOPICS LIST** — from **absolute basic → advanced → expert level**.
✅ **Only topic names**
✅ **Nothing missed**
✅ **Interview + real-world + production ready**
...
Here, you don’t need to install `create-react-app` globally.
...
Browsers don’t understand JSX directly → Babel converts JSX into normal JS.
...
**Answer:**
JSX (JavaScript XML) is a syntax used in React that allows you to write HTML-like code inside JavaScript. It makes UI code easier to read and write. JSX is not valid JavaScript by itself — it gets converted into normal JavaScript by Babel.
...
If condition true → element show
If false → nothing show
...
**Answer:**
Props (short for *properties*) are used to pass data from one component to another in React. Props are read-only and flow in one direction: from parent → child. They allow components to be reusable and dynamic.
...
State lives in parent → shared by children.
...
Example: form submit reloads page → prevent it.
...
**Answer:**
Event bubbling means events propagate from child → parent.
...
If items reorder → React may update wrong element.
...
**Answer:**
In class components, lifecycle methods are special methods that run at different stages of a component’s life. These stages are:
...
```
App → Context → Any component can access directly
```
...
When user visits `/about` → About component shows.
...
It does NOT catch rendering errors in components. That’s what error boundaries are for.
...
**Answer:**
By default, refs cannot be passed to custom components. To allow a parent component to access a child’s DOM node, we use `forwardRef`.
...
**Answer:**
Mocking APIs means simulating API responses during tests so you don’t call real servers.
...
If UI changes → snapshot mismatch.
...
Example scenario:
User types in search → React updates input immediately while preparing list rendering in background.
...
Developers don’t directly interact with Fiber, but it powers React internally.
...
3. React attaches JS → becomes interactive

Without hydration → static HTML only.
...
```
Send header → send sections → send rest
```
...
---
## 🟢 29. Real-World & Interview Topics
...
**Answer:**
Props drilling happens when data is passed from a parent component to deeply nested child components through multiple intermediate components that don’t actually need the data.
...
State lives in parent → shared across children.
...
1. React.memo → avoid re-render
2. useMemo → cache values
...
Now before commit → tests run automatically.
